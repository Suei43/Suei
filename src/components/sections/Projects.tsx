...
const Projects = forwardRef((props: any, ref: any) => {
  ...
  return (
    <InView as="div" onChange={(inView, entry) => handleInView(inView, entry)}>
      <section
        className={`w-full min-h-screen flex flex-col items-center justify-center gap-y-10`}
        ref={ref}
      >
        ...
        <div className="w-11/12 p-2 md:p-0 flex flex-row flex-wrap items-center gap-y-10 md:gap-x-5 md:justify-evenly">
          {projects.map((item: any) => {
            return (
              <div
                className={`p-4 rounded-lg md:w-3/12 h-[400px] border border-primary flex flex-col gap-y-4 relative items-center proj-card gradient-border`}
                key={item.id}
              >
                {item.image && (
                  <Image
                ...
                <div
                  className={`${item.image && " w-full "} p-0 md:p-1 flex flex-col gap-y-3`}
                >
        ...
        {viewMore && (
          <div className="w-11/12 p-2 md:p-0 flex flex-row flex-wrap items-center gap-y-10 md:gap-x-5 md:justify-evenly">
            {projects.map((item: any) => {
              return (
                <div
                  className={`p-4 rounded-lg md:w-3/12 h-fit border border-primary flex flex-col gap-y-4 relative items-center proj-card gradient-border`}
                  key={item.id}
                >
                  {item.image && (
                    <Image
